module UserAuth
  ...
  # access tokenの有効期限　JWTの初期値を設定する
  # UserAuth.access_token_lifetimeでアクセスすることができる = 30.minute
  ...
  # cookies[UserAuth.sessionkey]でrefreshトークンにアクセス
  mattr_accessor :session_key
  self.session_key = :refresh_token

  # userを識別するクレーム名
  # payloadにuserの識別情報を埋め込むsubというクレームに埋め込む
  mattr_accessor :user_claim
  self.user_claim = :sub

  # JWTの発行者を識別する文字列(認可サーバーURL)
  # ローカルの場合はlocalhost:3000(バックエンドサーバー)
